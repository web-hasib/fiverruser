...
export function DataTable<T>({ columns, data, className }: DataTableProps<T>) {
  return (
    <div
      className={cn(
        "rounded-2xl bg-primary overflow-hidden border border-primary-foreground/20",
        className,
      )}
    >
      <div className="overflow-x-auto scrollbar-hide">
        <Table className="w-full border-collapse">
          <TableHeader className="bg-primary-foreground/10">
            <TableRow className="border-b border-primary-foreground/20 hover:bg-transparent">
          ...
          <TableBody>
            {data.length > 0 ? (
              data.map((item, rowIndex) => (
                <TableRow
                  key={rowIndex}
                  className="border-b border-primary-foreground/20 last:border-0 hover:bg-primary-foreground/5 transition-colors"
                >

...
const NewCasePageContent = ({ id }: { id: string }) => {
  ...
  const [recordingModalType, setRecordingModalType] = useState<"start" | "end">("start");
  const [previousRecordingState, setPreviousRecordingState] = useState(false);
  ...
  const handleRecordButtonClick = () => {
    if (isRecording) {
      ...
      // If not recording, show start confirmation modal
  ...
  // Detect when recording actually starts (after confirmation)
  React.useEffect(() => {
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              <div className={cn(
                "flex bg-blue-600 rounded-lg overflow-hidden shadow-lg shadow-blue-600/20 h-10 transition-all",
                isRecording ? "w-fit" : "w-auto"
              )}>
                {/* Pause Button - Only show during recording */}
                {isRecording && (
                  <Button
                    onClick={isPaused ? resumeRecording : pauseRecording}
                    className="bg-blue-700 hover:bg-blue-800 text-white font-semibold flex items-center gap-1 px-2 h-10 rounded-none border-r border-blue-500/50 transition-all flex-shrink-0"
                ...
                <Button
                  ...
                  className={cn(
                    "bg-transparent hover:bg-blue-700 text-white font-semibold flex items-center gap-2 px-4 h-10 rounded-none transition-all whitespace-nowrap border-r border-blue-500/50",
                    isRecording ? "bg-blue-700 min-w-[100px]" : "min-w-[160px]",
                  )}
                >
                  <Mic
                    className={cn("size-4 flex-shrink-0", isRecording && "text-red-400")}
                  />

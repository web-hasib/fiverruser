...
export function LanguageSwitcher({className}: {className?: string}) {
  ...
  return (
    <div className="relative">
      <div
        ref={buttonRef}
        className={`flex items-center gap-1 md:gap-2 bg-secondary rounded-full px-1 md:px-3 py-1 md:py-2 cursor-pointer border border-border transition-colors hover:bg-secondary/80 notranslate text-foreground ${className}`}
        onClick={() => setIsOpen(!isOpen)}
      >
        <span className="flex items-center text-xs md:text-lg leading-none select-none">
        ...
        <span className="text-[8px] md:text-sm font-medium uppercase">
          {selectedLanguage}
        </span>
        <ChevronDown className="w-2 h-2 md:w-4 md:h-4 text-muted-foreground" />
      </div>
